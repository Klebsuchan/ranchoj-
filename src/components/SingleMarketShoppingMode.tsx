          ...
          <div className="space-y-2">
            {marketListItems.map((item) => (
              <div
                ...
                className={`p-3.5 rounded-2xl border-2 transition flex items-center justify-between gap-3 ${
                  item.isBought
                    ? 'bg-slate-100/80 border-slate-300 text-slate-400 opacity-75'
                    : 'bg-white border-slate-200 hover:border-red-300 text-slate-800 shadow-xs'
                }`}
              >
                {/* Large Checkbox for physical cart checklist (Elderly-friendly touch target) */}
                <button
                  type="button"
                  onClick={() => onToggleBought(item.id)}
                  className={`min-h-[44px] min-w-[44px] rounded-xl border-2 flex items-center justify-center shrink-0 transition active:scale-95 ${
                    item.isBought
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-1.5">
                    <span className={`text-base font-extrabold truncate ${item.isBought ? 'line-through text-slate-400' : 'text-slate-900'}`}>
                      {item.name}
                    </span>
                  </div>
                  <div className="flex items-center gap-2 mt-0.5 text-xs text-slate-600">
                    ...
                    <span>• Total: <strong className="text-red-600 text-sm font-black">R$ {item.itemTotal.toFixed(2)}</strong></span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  <div className="flex items-center border-2 border-slate-200 rounded-xl overflow-hidden bg-slate-50">
                    <button
                      ...
                      className="w-9 h-9 flex items-center justify-center text-slate-700 hover:bg-slate-200 active:scale-90 font-black text-lg"
                      title="Diminuir"
                    >
                      -
                    </button>
            ...
            <div className="pt-2 grid grid-cols-1 sm:grid-cols-3 gap-2">
              <button
                type="button"
                onClick={handleExportPdf}
                className="min-h-[48px] py-2.5 px-3.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-extrabold text-xs flex items-center justify-center gap-2 transition shadow-xs active:scale-98"
              ...
              <button
                ...
                className="min-h-[48px] py-2.5 px-3.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs flex items-center justify-center gap-2 transition shadow-xs active:scale-98"
                title="Mandar lista pronta no WhatsApp da família"
              >
              ...
              {onSaveToHistory && (
                <button
                  ...
                  className="min-h-[48px] py-2.5 px-3.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold text-xs flex items-center justify-center gap-2 transition shadow-2xs active:scale-98 border border-slate-200"
                >
