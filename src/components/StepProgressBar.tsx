          ...
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center py-2 px-1 rounded-xl transition relative active:scale-95 ${
                isActive
                  ...
                  : isCompleted
                  ? 'bg-emerald-50/80 text-emerald-900 hover:bg-emerald-100/70 border border-emerald-200/60'
            ...
            >
              ...
              <div className="flex items-center gap-1">
                ...
                <Icon className={`w-3.5 h-3.5 ${isActive ? 'text-emerald-100' : isCompleted ? 'text-emerald-700' : 'text-slate-400'}`} />
              </div>

              {/* Step Label */}
              <span className={`text-[11px] font-bold mt-1 tracking-tight truncate max-w-full ${
                isActive ? 'text-white' : isCompleted ? 'text-emerald-950' : 'text-slate-600'
              }`}>
                {step.title}
              </span>
      ...
      <div className="mt-2 pt-2 border-t border-slate-100 flex items-center justify-between text-[11px]">
        ...
        {budgetLimit > 0 && (
          <div className="flex items-center gap-1 font-mono text-[10px] shrink-0">
            ...
            <span className={`font-bold ${totalRancho > budgetLimit ? 'text-rose-600 font-extrabold' : 'text-emerald-700'}`}>
